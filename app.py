...
def preprocess_image(image_data):
    ...
    try:
        print(f"[DEBUG] preprocess_image called with type: {type(image_data)}")
        print(f"[DEBUG] image_data length: {len(image_data) if isinstance(image_data, str) else 'N/A'}")
        print(f"[DEBUG] image_data starts with: {str(image_data)[:50] if isinstance(image_data, str) else 'N/A'}")
        
        # Handle base64 encoded image
        if isinstance(image_data, str) and image_data.startswith('data:image'):
            print("[DEBUG] Detected data:image format, extracting base64...")
            # Extract base64 string
            image_data = image_data.split(',')[1]
            print(f"[DEBUG] Base64 extracted, length: {len(image_data)}")
            image_bytes = base64.b64decode(image_data)
            print(f"[DEBUG] Decoded to {len(image_bytes)} bytes")
            image = Image.open(BytesIO(image_bytes))
            print(f"[DEBUG] Image opened, mode: {image.mode}, size: {image.size}")
        else:
            print("[DEBUG] Not a data:image format, trying direct decode...")
        ...
        print(f"[DEBUG] Resizing to {IMG_SIZE}x{IMG_SIZE}")
        ...
        print(f"[DEBUG] Final array shape: {image_array.shape}")
        return image_array
    except Exception as e:
...
def is_leaf_image(image_array):
    """
    Validate that image is a real leaf (not blank, not solid color)
    Only reject completely empty/uniform images
    """
    try:
        if image_array is None or image_array.size == 0:
            print("[DEBUG] Image array is None or empty")
            return False
        
        # Check for completely uniform/blank images (std dev near 0)
        std_dev = np.std(image_array)
        print(f"[DEBUG] Image std dev: {std_dev:.4f}")
        
        if std_dev < 0.01:
            print("[DEBUG] Image is too uniform (blank/solid color) - REJECTED")
            return False
        
        print(f"[DEBUG] Image accepted - shape: {image_array.shape}, std: {std_dev:.4f}")
        return True
    except Exception as e:
...
def detect_disease():
    ...
    if request.method == 'OPTIONS':
        print("[INFO] CORS preflight received")
        return '', 200
...
def handle_400(e):
    print(f"[ERROR] 400 Bad Request: {str(e)}")
    return jsonify({'error': str(e)}), 400

@app.errorhandler(500)
def handle_500(e):
    print(f"[ERROR] 500 Server Error: {str(e)}")
